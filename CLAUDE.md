...
├── Procfile                           # Process entry for hosted deploys
├── _config.yml                        # Jekyll theme config (legacy Pages; unused once Pages source = GitHub Actions)
├── .gitignore
│
├── config/
...
│
├── docs/                              # GitHub Pages output — regenerated by dashboard.yml, committed by Actions
│   ├── .nojekyll                      # Stops Jekyll from mangling the generated HTML
│   ├── index.html                     # Latest generated dashboard
│   ├── latest.json                    # Machine-readable snapshot of the latest run
│   └── archive/                       # One HTML per run, YYYY-MM-DD-<slot>.html
│
├── src/
│   ├── __init__.py
│   ├── models.py              [216L]  # Data classes: Pivot, Wave, WaveCount, FibLevel, Alert, Divergence, etc.
│   ├── data_feed.py           [257L]  # OHLCV fetching + RSI/MACD/Volume SMA + caching + rate limiting
│   ├── options_feed.py        [167L]  # Yahoo Finance options chain fetcher (v7 endpoint)
│   ├── greeks.py              [ 82L]  # Black-Scholes Greeks — fills in delta where Yahoo doesn't provide it
...
│   ├── scanner.py             [192L]  # Multi-ticker Wave Options Scanner orchestrator
│   ├── signal_builder.py      [294L]  # Build + validate canonical signal JSON (cross-scanner contract)
│   ├── strategy_selector.py   [246L]  # Options structure from wave direction, IV rank, DTE
│   ├── persistence.py         [141L]  # Signals to daily JSONL files + SQLite
│   ├── discord_webhook.py     [132L]  # Discord webhook delivery — rich embed formatting
│   ├── api.py                 [113L]  # FastAPI app: GET /api/signals + manual scan trigger
...
│
├── scripts/
│   ├── run_scanner.py         [ 91L]  # Main entry point / CLI orchestrator
│   ├── demo_pipeline.py       [147L]  # End-to-end pipeline demo on saved Yahoo fixtures
...
│   │   ├── yahoo_chart_sample.json    # Saved Yahoo chart response for offline tests
│   │   └── yahoo_options_sample.json  # Saved Yahoo options chain response for offline tests
...
│   ├── test_data_feed.py      [ 60L]  # OHLCV fetch + indicator column tests
│   ├── test_options_feed.py   [ 47L]  # Options chain parsing tests
│   ├── test_greeks.py         [ 44L]  # Black-Scholes delta tests
│   ├── test_signal_builder.py [107L]  # Signal JSON build + schema validation tests
│   ├── test_strategy_selector.py [60L] # Strategy selection tests
│   ├── test_persistence.py    [ 76L]  # JSONL + SQLite persistence tests
│   ├── test_discord_webhook.py [70L]  # Discord embed formatting tests
│   └── test_api.py            [ 85L]  # FastAPI endpoint tests (needs httpx)
│
└── .github/
    └── workflows/
        ├── scanner.yml                # GitHub Actions: scan, test, lint (ruff), typecheck (mypy)
        └── dashboard.yml              # GitHub Actions: regenerate docs/ twice each weekday, commit, deploy Pages
```
...
3. **`run_scanner.py`** — slimmed to 91 lines in the multi-ticker refactor; no longer an issue
